...
public class IASTPrinter extends IASTBaseVisitor {
	...
	public static final String KEY_INNER_USE_VARIABLES = "innerUseVariables";
	...
	public Object visitFunctionExpression(IASTFunctionExpression node) {
		...
		// innerUseVariables
		JsonArrayBuilder jaInnerUseVariables = Json.createArrayBuilder();
		for (String local : node.innerUseVariables) {
			jaInnerUseVariables.add(local);
		}
		jb.add(KEY_INNER_USE_VARIABLES, jaInnerUseVariables);
		// body
		jb.add(KEY_BODY, (JsonObject) node.body.accept(this));
