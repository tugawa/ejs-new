...
public class CodeGenerator extends IASTBaseVisitor {
    ...
    static class Environment {
        ...
        void openFrame(List<String> args, List<String> locals) {
            frameList.addFirst(new Frame(args, locals));
        }
        ...
        int getNumberOfGPRegisters() {
        	return this.frameList.getFirst().getNumberOfGPRegisters();
        }
        
        int getNumberOfArgumentRegisters() {
        	return this.frameList.getFirst().getNumberOfArgumentRegisters();
        }
    ...
    public BCBuilder compile(IASTProgram node) {
        ...
        try {
            ...
            bcBuilder.setNumberOfRegisters(env.getNumberOfGPRegisters(), env.getNumberOfArgumentRegisters());
            env.closeFrame();
    ...
    public Object visitNewExpression(IASTNewExpression node) {
        ...
        Register[] argRegs = env.freshArgumentRegister(node.arguments.size() + 1);
        {
            ...
            for (IASTExpression arg : node.arguments) {
                Register tmpReg = env.getCurrentFrame().freshRegister();
                tmpRegs.add(tmpReg);
