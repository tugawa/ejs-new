...
public class TypeCheckVisitor extends TreeVisitorMap<DefaultVisitor> {
    ...
    public void start(Tree<?> node) {
        try {
            ...
            Stack s = new Stack();
            for (Tree<?> chunk : node) {
                dict = visit((SyntaxTree)chunk, dict, s);
            }
    ...
    private final TypeMap visit(SyntaxTree node, TypeMap dict, Stack s) throws Exception {
        return find(node.getTag().toString()).accept(node, dict, s);
    }

    public class DefaultVisitor {
        public TypeMap accept(SyntaxTree node, TypeMap dict, Stack s) throws Exception {
            return dict;
        }
    }
    ...
    public class FunctionMeta extends DefaultVisitor {
        ...
        public TypeMap accept(SyntaxTree node, TypeMap dict, Stack s) throws Exception {
            ...
            return visit((SyntaxTree)body, dict, s);
        }
    }
    ...
    public class Block extends DefaultVisitor {
        ...
        public TypeMap accept(SyntaxTree node, TypeMap dict, Stack s) throws Exception {
            for (Tree<?> seq : node) {
                dict = visit((SyntaxTree)seq, dict, s);
            }
            return dict;
        }
    }
    ...
    public class Return extends DefaultVisitor {
        @Override
        public TypeMap accept(SyntaxTree node, TypeMap dict, Stack s) throws Exception {
            return visit(node.get(0), dict, s);
        }
    }
    ...
    public class Assignment extends DefaultVisitor {
        ...
        public TypeMap accept(SyntaxTree node, TypeMap dict, Stack s) throws Exception {
            ...
            AstType rhsType = visit(right, dict, s).getExprType();
    ...
    public class Declaration extends DefaultVisitor {
        ...
        public TypeMap accept(SyntaxTree node, TypeMap dict, Stack s) throws Exception {
            ...
            AstType rhsType = visit(expr, dict, s).getExprType();
            var.setType(rhsType);
    ...
    public class If extends DefaultVisitor {
        ...
        public TypeMap accept(SyntaxTree node, TypeMap dict, Stack s) throws Exception {
            ...
            TypeMap thenDict = visit(thenNode, dict, s);
            SyntaxTree elseNode = node.get(Symbol.unique("else"));
            TypeMap elseDict = visit(elseNode, dict, s);
