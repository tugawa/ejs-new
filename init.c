...
void init_global_constants(void) {
  ...
  // set_obj_prop(gconsts.g_object_proto, "prototype", gconsts.g_object_proto, ATTR_ALL);
...
#ifdef PARALLEL
  set_obj_prop(gconsts.g_function_proto, "setAtomic", new_builtin(functionProtoSetAtomic, 0), ATTR_DE);
#endif
...
#ifdef PARALLEL
  set_obj_prop(gconsts.vmTest, "run", new_builtin(vmTestRun, 1), ATTR_DE);
  set_obj_prop(gconsts.thread, "init", new_builtin(threadInit, 1), ATTR_DE);
  set_obj_prop(gconsts.tcp, "init", new_builtin(tcpInit, 0), ATTR_DE);
#endif
